...
//    @Scheduled(cron = "0/5 * * * * ? ")
    ...
    public void syncAccount1(){
        try {
            ...
            String SQL="select id from account where (customItem201__c is null or customItem201__c = '') and approvalStatus = 3";
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                try {
                    ...
                    String EndUserTypeTemp = EndUserType;
...
//                JSONArray jsonArray1 = JSONArray.parseArray("[\"CustNum\",\"CustSeq\",\"Name\",\"Addr_1\",\"Addr_2\",\"Addr_3\",\"Addr_4\",\"Country\",\"State\",\"City\",\"County\",\"Zip\",\"CustType\",\"CreditLimit\",\"TermsCode\",\"CreditHold\",\"EndUserType\",\"TerritoryCode\",\"SalesTeamID\",\"cusUf_GlobalId\",\"ShowInDropDownList\",\"CusShipmentApprovalRequired\",\"IncludeTaxInPrice\",\"CurrCode\",\"TaxCode1\"]");
                    ...
                    for (int j = 0; j < ERPDataArray.size(); j++) {
                        ...
                        if (StringUtils.isBlank(EndUserTypeTemp)){
                            String s = updateCustomerERPNoError(ERPDataId,"最终用户类型不能为空");
                            continue;
                        }
                        ...
                        if (StringUtils.isBlank(slCustomers)){
                            ...
                            updateAccountERPNo(accountId, CustNum);
                    ...
                    String s = updateCustomerERPNoError(ERPDataID, e.getMessage().length()>300?e.getMessage().substring(0, 300):e.getMessage());
                }
            }
...
//           String InvCategory=getArrayToData(jsonObject,"customItem15__c");//发票类别
     ...
     * @throws Exception
     */
    public String updateCustomerERPNoError(Long ERPDataId,String customNum) throws Exception {
        JSONObject object=new JSONObject();
        object.put("id",ERPDataId);
        object.put("customItem5__c",customNum);
        String post = queryServer.updateCustomizeByIdNoThrowException(object);
        return post;
    }
    /**
