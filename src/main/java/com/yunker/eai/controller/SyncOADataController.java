...
import com.yunker.eai.oaPackage.HXCRMService;
...
//            bulkAPI.createDataTaskJob(productArray, "product", "insert");
     ...
     */
    @SneakyThrows
    @RequestMapping("/initUSAERPAccount")
    public void initUSAERPAccount() {
        JSONArray ERPArray = new JSONArray();
        Map<String, JSONArray> ERPDataMap = new HashMap<>();
        Map<Long, String> accountERPMap = new HashMap<>();
        JSONArray jsonArray = new JSONArray();
        jsonArray.add("LIVE_SDUSA");
//        jsonArray.add("LIVE_HXYLQX");
//        jsonArray.add("LIVE_HXLMD");
...
//        jsonArray.add("LIVE_HXSW");
        ...
        //查询所有账套
        String sql = "select id,customItem4__c from customEntity63__c where customItem3__c = 1";
        JSONObject bySql = queryServer.getByXoqlSimple(sql);
        JSONArray all = queryServer.getAllByXoqlSample(getToken(), bySql, sql);
        for (int i = 0; i < all.size(); i++) {
            JSONObject jsonObject = all.getJSONObject(i);
            Long customItem2__c = jsonObject.getLong("customItem4__c");//客户id
            accountERPMap.put(customItem2__c, "success");
        }
        ...
        for (Map.Entry<String, JSONArray> item : ERPDataMap.entrySet()) {
            ...
            for (int i = 0; i < value.size(); i++) {
                ...
                Long ownerId = account.getLong("ownerId");
                String customItem201__c = account.getString("customItem201__c");
                Integer integer = object.getInteger(config);
                if (integer == null || integer == 0) {
                    continue;
                }
                if (accountId != null && accountId != 0) {
                    String s = accountERPMap.get(accountId);
                    if (StringUtils.isBlank(s)) {
                        JSONObject erpObject = new JSONObject();
                        erpObject.put("entityType", 1340810097181017L);
                        erpObject.put("name", config);
                        erpObject.put("customItem3__c", integer);
                        erpObject.put("customItem4__c", accountId);
                        erpObject.put("ownerId", ownerId);
    ...
    /*
     * @Description TODO 历史数据初始化默认账套
     * @author lucg.
     * @date 2020/12/23 15:22
     */
    @RequestMapping("/initAccountCRM")
    public void initAccountCRM() throws Exception {
        JSONArray ERPArray = new JSONArray();

        Map<Long, String> accountERPMap = new HashMap<>();

        //查询所有账套
        String sql = "select id,customItem4__c from customEntity63__c";
        JSONObject bySql = queryServer.getByXoqlSimple(sql);
        JSONArray all = queryServer.getAllByXoqlSample(getToken(), bySql, sql);
        for (int i = 0; i < all.size(); i++) {
            JSONObject jsonObject = all.getJSONObject(i);
            Long customItem2__c = jsonObject.getLong("customItem4__c");//客户编号
            accountERPMap.put(customItem2__c, "success");
        }
        ...
        for (int i = 0; i < all1.size(); i++) {
            ...
            String customItem201__c = jsonObject.getString("customItem201__c");
            String s = accountERPMap.get(id);
            if (StringUtils.isBlank(s)) {
                JSONObject erpObject = new JSONObject();
                if (StringUtils.isBlank(customItem201__c)) {
                    erpObject.put("entityType", 1340810097181017L);
                    erpObject.put("name", config);
                    erpObject.put("customItem3__c", 6);
                    erpObject.put("customItem4__c", id);
                    erpObject.put("ownerId", ownerId);
                } else {
                    erpObject.put("entityType", 1340810097181017L);
                    erpObject.put("name", config);
    ...
    /*
     * @Description TODO 从erp同步到CRM
     * @author lucg.
     * @date 2020/12/3 11:54
     */
    @RequestMapping("/initAddressData")
    public void initAddressData() throws Exception {
...
//        jsonArray.add("LIVE_HXSW");
        ...
        String accountSql = "select id,customItem2__c,customItem7__c,customItem4__c,customItem11__c,customItem10__c  from customEntity63__c";
        JSONObject bySql = queryServer.getByXoqlSimple(accountSql);
        JSONArray all = queryServer.getAllByXoqlSample(getToken(), bySql, accountSql);
        for (int i = 0; i < all.size(); i++) {
            JSONObject jsonObject = all.getJSONObject(i);
        ...
        for (Map.Entry<String, JSONArray> item : ERPMap.entrySet()) {
            ...
            for (int i = 0; i < value.size(); i++) {
                ...
                String CustNum = jsonObject.getString("CustNum");
                String Name = jsonObject.getString("Name");
                ERPDataMap.put(config + "," + CustNum + "," + Name, "success");
            }
        }
        ;
        for (int i = 0; i < addressArray.size(); i++) {
            ...
            String name = jsonObject.getString("name");
            String s = ERPDataMap.get(config + "," + accountNum + "," + name);
            addressExisitMap.put(config + "," + accountNum + "," + name, "success");
            if (StringUtils.isBlank(s)) {
        ...
        for (Map.Entry<String, JSONArray> item : ERPMap.entrySet()) {
            ...
            for (int i = 0; i < value.size(); i++) {
                ...
                if (StringUtils.isBlank(s)) {//不存在，创建
                    ...
                    String accountName = accountERP.getString("customItem10__c");
                    if (StringUtils.isBlank(Name)) {
                        Name = accountName;
                    }
                    Long accountOwnerId = allUsers.get(accountOwner);
                    if (accountId == null || accountId == 0 || erpId == null || erpId == 0) {
                        continue;
                    }
...
//        }
     ...
     * @return
     */
    @RequestMapping("/initReceiptAddress")
    @ResponseBody
...
//        jsonArray.add("LIVE_HXSW");
        ...
        String ERPAccountSql = "select id,customItem4__c,customItem2__c,customItem7__c,name,customItem10__c,customItem11__c from customEntity63__c";
        ...
        for (int i = 0; i < allByXoqlSample3.size(); i++) {
            ...
            ERPAccountMap.put(customItem2__c + "," + customItem7__c, jsonObject);
        }
        ...
        String addressSql = "select id,name,customItem1__c,customItem11__c from customEntity9__c";
        ...
        for (int i = 0; i < allByXoqlSample.size(); i++) {
            ...
            String name = jsonObject.getString("name");
            addressMap.put(accountId + "," + erpId + "," + name, id);
        }
        ...
        for (int i = 0; i < jsonArray.size(); i++) {
            String config = jsonArray.getString(i);
            String ERPtoken = idoWebServiceSoap.createSessionToken(userId, pswd, config);
            String result = idoWebServiceSoap.loadJson(ERPtoken, "SLShipTos", "DropShipNo,DropSeq,Name,addr_1,addr_2,addr_3,addr_4,Country,State,City,Contact,Phone", "", "", "", 300000);
            JSONObject resultJson = JSONObject.parseObject(result);
            JSONArray propertyList = resultJson.getJSONArray("PropertyList");
            JSONArray Items = resultJson.getJSONArray("Items");
            JSONArray propertyArray = new JSONArray();
        ...
        for (Map.Entry<String, JSONArray> item : ERPMap.entrySet()) {
            ...
            for (int i = 0; i < value.size(); i++) {
                ...
                String address4 = jsonObject.getString("addr_4");
                String country = jsonObject.getString("Country");
                String province = jsonObject.getString("State");
                String city = jsonObject.getString("City");
                String contact = jsonObject.getString("Contact");
                String phone = jsonObject.getString("Phone");
                JSONObject ERPObject = ERPAccountMap.get(accountNo + "," + config);
                if (ERPObject == null) {
                    continue;
                }
...
//                String accountName = ERPObject.getString("customItem10__c");
                ...
                Long aLong = addressMap.get(accountId + "," + erpId + "," + Name);
     ...
     * @return
     */
    @RequestMapping("/initBankInfo")
    @ResponseBody
    public String initBankInfo() throws Exception {
        JSONArray jsonArray = new JSONArray();
        jsonArray.add("LIVE_SDUSA");
//        jsonArray.add("LIVE_HXYLQX");
//        jsonArray.add("LIVE_HXLMD");
...
//        jsonArray.add("LIVE_BJHY");
//        jsonArray.add("LIVE_HXSW");
        JSONObject TaxCodeObject = JSONObject.parseObject("{\n" +
                "  \"13\":1,\n" +
                "  \"0\":2\n" +
                "}");
        ...
        Map<String, JSONObject> ERPAccountMap = new HashMap<>();
        Map<String, Long> addressMap = new HashMap<>();
        Map<String, Integer> payTypeMap = new HashMap<>();
        Map<String, JSONArray> ERPMap = new HashMap<>();
        ...
        for (int i = 0; i < allByXoqlSample3.size(); i++) {
            ...
            ERPAccountMap.put(customItem201__c, jsonObject);
        }

        String fieldsByBelongId = queryServer.getFieldsByBelongId(729627745665251L);
        JSONObject object1 = JSONObject.parseObject(fieldsByBelongId);
        JSONArray fields = object1.getJSONArray("fields");
        for (int i = 0; i < fields.size(); i++) {
            JSONObject jsonObject = fields.getJSONObject(i);
            String propertyname = jsonObject.getString("propertyname");
            if ("customItem14__c".equals(propertyname)) {
                JSONArray selectitem = jsonObject.getJSONArray("selectitem");
                for (int j = 0; j < selectitem.size(); j++) {
                    JSONObject jsonObject1 = selectitem.getJSONObject(j);
                    Integer value = jsonObject1.getInteger("value");
                    String lable = jsonObject1.getString("label");
                    payTypeMap.put(lable, value);
                }
            }
        }
        ...
        for (int i = 0; i < allByXoqlSample.size(); i++) {
            ...
            Long accountId = jsonObject.getLong("customItem1__c");
            String name = jsonObject.getString("name");
            addressMap.put(accountId + "," + name, id);
        }
        ...
        for (int i = 0; i < jsonArray.size(); i++) {
            String config = jsonArray.getString(i);
            String ERPtoken = idoWebServiceSoap.createSessionToken(userId, pswd, config);
            String result = idoWebServiceSoap.loadJson(ERPtoken, "SLCustomers", "CustNum,cusUf_Bank,cusUf_BankAcct,cusUF_ReservedField1,cusUf_Note,PayType,TaxCode1,BankCode,Charfld3,Charfld2", "", "", "", 300000);
            JSONObject resultJson = JSONObject.parseObject(result);
            JSONArray propertyList = resultJson.getJSONArray("PropertyList");
            JSONArray Items = resultJson.getJSONArray("Items");
            JSONArray propertyArray = new JSONArray();
        ...
        for (Map.Entry<String, JSONArray> item : ERPMap.entrySet()) {
            ...
            for (int i = 0; i < value.size(); i++) {
                JSONObject jsonObject = value.getJSONObject(i);
                String accountNo = jsonObject.getString("CustNum");//客户编号
                String cusUf_Bank = jsonObject.getString("cusUf_Bank");//开户行
                String cusUf_BankAcct = jsonObject.getString("cusUf_BankAcct");//银行账户
                String cusUF_ReservedField1 = jsonObject.getString("cusUF_ReservedField1");//纳税人识别号
                String cusUf_Note = jsonObject.getString("cusUf_Note");//开票特殊要求
                String PayType = jsonObject.getString("PayType");//付款类型
                String TaxCode1 = jsonObject.getString("TaxCode1");//税率
                String BankCode = jsonObject.getString("BankCode");//银行码
                String Charfld3 = jsonObject.getString("Charfld3");//开票电话
                String Charfld2 = jsonObject.getString("Charfld2");//开票地址
                String bankCode = bankCodeJsonReverse.getString(BankCode);

                JSONObject ERPObject = ERPAccountMap.get(accountNo);
                if (ERPObject == null) {
                    continue;
                }
                Long accountId = ERPObject.getLong("id");
//                String accountName = ERPObject.getString("customItem10__c");
                Long ownerId = ERPObject.getLong("ownerId");
                String accountName = ERPObject.getString("accountName");
                Integer PayTypeInteger = payTypeMap.get(PayType);
...
//        jsonArray.add("LIVE_HXSW");
        ...
        Map<String, JSONObject> departMap = new HashMap<>();
        ...
        String departSql = "select id,customItem126__c,departName from department";
        ...
        for (int i = 0; i < allDepart.size(); i++) {
            ...
            departMap.put(customItem126__c, jsonObject);
        }
        String ERPAccountSql = "select id,customItem4__c,customItem2__c,customItem7__c,name,customItem10__c,customItem11__c from customEntity63__c";
        JSONObject byXoqlSimple3 = queryServer.getByXoqlSimple(ERPAccountSql);
        JSONArray allByXoqlSample3 = queryServer.getAllByXoqlSample(getToken(), byXoqlSimple3, ERPAccountSql);
        ...
        for (int i = 0; i < jsonArray.size(); i++) {
            String config = jsonArray.getString(i);
            String ERPtoken = idoWebServiceSoap.createSessionToken(userId, pswd, config);
            String result = idoWebServiceSoap.loadJson(ERPtoken, "HXCustCredits", "custnum,bu,custgrade,custterm,custcredit", "", "", "", 300000);
            JSONObject resultJson = JSONObject.parseObject(result);
            JSONArray propertyList = resultJson.getJSONArray("PropertyList");
            JSONArray Items = resultJson.getJSONArray("Items");
            JSONArray propertyArray = new JSONArray();
        ...
        for (Map.Entry<String, JSONArray> item : ERPMap.entrySet()) {
            ...
            for (int i = 0; i < value.size(); i++) {
                ...
                String accountNo = jsonObject.getString("custnum");
                if ("KUS0009".equals(accountNo)) {
                    System.out.println(1);
                }
                String bu = jsonObject.getString("bu");
                String custgrade = jsonObject.getString("custgrade");
                String custterm = jsonObject.getString("custterm");
                Double custcredit = jsonObject.getDouble("custcredit");
                custcredit = custcredit == null ? 0 : custcredit;
                JSONObject object3 = departMap.get(bu);
                if (object3 == null) {
                    continue;
                }

                Long customItem6__c = object3.getLong("id");
                String name = object3.getString("departName");
                Integer customItem10__c = levelMap.get(custgrade);
                Long customItem8__c = zqMap.get(custterm);
                JSONObject object2 = ERPAccountMap.get(accountNo + "," + config);
                if (object2 == null) {
                    continue;
                }
                Long accountId = object2.getLong("customItem4__c");
                Long customItem13__c = object2.getLong("id");
                ...
                if (aLong == null || aLong == 0) {
                    existMap.put(accountId + "," + customItem13__c + "," + customItem6__c, 8888L);
                    JSONObject object = new JSONObject();
                    object.put("name", name);
                    object.put("customItem3__c", custcredit);
    ...
    public void updateAccountHightSea() throws Exception {
        String sql = "select id from account where createdBy = 1185240 and createdAt<1608710706000 and createdAt>1608541230000";
        JSONObject byXoqlSimple = queryServer.getByXoqlSimple(sql);
        JSONArray allByXoqlSample = queryServer.getAllByXoqlSample(getToken(), byXoqlSimple, sql);
        for (int i = 0; i < allByXoqlSample.size(); i++) {
            JSONObject jsonObject = allByXoqlSample.getJSONObject(i);
    ...
    public void syncAccount() throws Exception {
        ...
        Map<Long, Integer> RefNumMap = new HashMap<>();
        Map<String, Integer> DerFullNameMap = new HashMap<>();
        Map<String, Integer> SalesTeamIDMap = new HashMap<>();
...
//        jsonArray.add("LIVE_BJHY");
        ...
        Map<String, JSONArray> ERPDataMap = new HashMap<>();
        Map<String, Map<String, JSONArray>> IteamDataMap = new HashMap<>();
        Map<Long, Boolean> formalMap = new HashMap<>();
        Map<String, Long> codeMap = new HashMap<>();
        ...
        JSONObject byXoqlSimple = queryServer.getByXoqlSimple(sql);
        JSONArray allByXoqlSample = queryServer.getAllByXoqlSample(getToken(), byXoqlSimple, sql);
        Boolean customItem193__c = null;
        String customItem239__c = null;
        Long customItem230__c = null;
        for (int i = 0; i < allByXoqlSample.size(); i++) {
            JSONObject jsonObject = allByXoqlSample.getJSONObject(i);
            Long id = jsonObject.getLong("id");
//            Long ownerId = jsonObject.getLong("ownerId");
            Long ownerId = jsonObject.getLong("ownerId");
            String accountName = jsonObject.getString("accountName");
            customItem193__c = jsonObject.getBoolean("customItem193__c");
            String customItem201__c = jsonObject.getString("customItem201__c");
            customItem239__c = jsonObject.getString("customItem239__c");
            customItem230__c = jsonObject.getLong("customItem230__c");
            accountMap.put(accountName, id);
            ownerMap.put(id, ownerId);
            codeMap.put(customItem201__c, id);
            formalMap.put(id, customItem193__c);
            inforMap.put(id, customItem239__c);
        ...
        for (int i = 0; i < jsonArray.size(); i++) {
            ...
            String config = jsonArray.getString(i);
            String ERPtoken = idoWebServiceSoap.createSessionToken(userId, pswd, config);
            //销售团队SLSalesTeamMembers   refnum员工编号  derfullname员工姓名  salesteamid团队ID=客户编号
...
//            IteamDataMap.put(config, map);
        ...
        JSONArray accountAddArray = new JSONArray();
        JSONArray ERPArray = new JSONArray();
        JSONArray ERPUpdateArray = new JSONArray();
        for (Map.Entry<String, JSONArray> item : ERPDataMap.entrySet()) {//忽略
            String config = item.getKey();
            Integer erpNo = object.getInteger(config);
            JSONArray value = item.getValue();
            for (int i = 0; i < value.size(); i++) { //遍历所有ERP数据
                ...
                String uf_custype = jsonObject.getString("uf_custype");
                ...
                Date customItem210__c = new Date();     //成立日期
...
//                }
                ...
                if (accountId == null || accountId == 0) {//不存在
                    ...
                    account.put("entityType", 749177010471194L);
                    ...
                    account.put("customItem239__c", customItem239__c);
...
//                    account.put("customItem233__c", customItem233__c);
                    ...
                    if (userId != null && userId != 0) {
                        account.put("customItem230__c", userId);
                    }

                    accountId = codeMap.get(CustNum);
                    if (accountId == null || accountId == 0) {
...
//                            updateObject.put("customItem214__c", customItem214__c);
...
//                            updateObject.put("customItem193__c", customItem193__c);
//                            updateObject.put("customItem239__c", customItem239__c);
//
...
//                        }
                    ...
                    if (accountId != null && accountId != 8888L) {
...
//                        accountArray.add(updateObject);
                        ...
                        if (accountId != null && accountId != 8888L) {
                            ...
                            if (erpId == null || erpId == 0) {
                                ...
                                JSONObject updateObject = new JSONObject();
                                updateObject.put("id", accountId);
                                updateObject.put("customItem186__c", customItem186__c);//经营渠道 单选
                                updateObject.put("customItem199__c", customItem199__c);//线上线下 单选
                                updateObject.put("customItem207__c", customItem207__c);//一级机构 单选
                                updateObject.put("customItem208__c", customItem208__c);//二级
                                updateObject.put("customItem209__c", customItem209__c);//三级
                                updateObject.put("customItem214__c", customItem214__c);//是否停止合作 单选
                                if (StringUtils.isNotBlank(uf_createdate)) {
                                    updateObject.put("customItem210__c", customItem210__c);
                                }
                                updateObject.put("customItem211__c", customItem211__c);
                                updateObject.put("customItem212__c", customItem212__c);
                                updateObject.put("customItem213__c", customItem213__c);//近三年诉讼 单选
//                                updateObject.put("customItem201__c", customItem201__c);
//                                updateObject.put("customItem193__c", customItem193__c);
...
////            }
        ...
        if (saleArray.size() > 0) {
//            bulkAPI.createDataTaskJob(saleArray, "account", "update");
//            for (int i = 0; i < 20; i++) {
//                JSONObject jsonObject = saleArray.getJSONObject(i);
//                String s =queryServer.updateAccount(jsonObject);
//                System.out.println(s);
//            }
        }
...
//        }
    ...
    @ResponseBody
    public void syncAccount1() throws Exception {
        DateFormat df = new SimpleDateFormat("yyyyMMdd HH:mm:ss.sss");
        ...
        Map<String, Integer> DerFullNameMap = new HashMap<>();
        Map<String, Integer> SalesTeamIDMap = new HashMap<>();
        Map<String, Integer> ReservedMap = new HashMap<>();
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            if ("customItem194__c".equals(propertyname)) {
                JSONArray selectitem = jsonObject.getJSONArray("selectitem");
                for (int j = 0; j < selectitem.size(); j++) {
                    JSONObject jsonObject1 = selectitem.getJSONObject(j);
                    Integer value = jsonObject1.getInteger("value");
                    String lable = jsonObject1.getString("label");
                    ReservedMap.put(lable, value);
                }
            }
...
//        jsonArray.add("LIVE_BJHY");
        ...
        //查询所有客户
        String sql = "select id,accountName,ownerId,customItem193__c ,customItem201__c ,customItem230__c,customItem239__c from account where createdBy = 1185240 and createdAt<1608710706000 and createdAt>1608541230000";
        JSONObject byXoqlSimple = queryServer.getByXoqlSimple(sql);
        JSONArray allByXoqlSample = queryServer.getAllByXoqlSample(getToken(), byXoqlSimple, sql);
        Boolean customItem193__c = null;
        String customItem239__c = null;
...
//            Long ownerId = jsonObject.getLong("ownerId");
        ...
        for (Map.Entry<String, JSONArray> item : ERPDataMap.entrySet()) {//忽略
            ...
            for (int i = 0; i < value.size(); i++) { //遍历所有ERP数据
                ...
                Long entityType = uf_domforObject.getLong(uf_domfor);
                Integer customItem194__c = ReservedMap.get(ReservedField1);
                Integer customItem186__c = uf_salewayMap.get(uf_saleway);
                Integer customItem199__c = uf_ifonlineMap.get(uf_ifonline);
                Integer customItem207__c = uf_org1Map.get(uf_org1);
                Integer customItem208__c = uf_org2Map.get(uf_org2);
...
//            jsonArray.add("LIVE_HXSW");
        ...
        String productSql = "select id,productName,customItem130__c from product";
...
//                String productName = jsonObject.getString("productName");
            ...
            productMap.put(customItem130__c, id);
        }
        ...
        for (Map.Entry<String, JSONArray> stringJSONArrayEntry : map.entrySet()) {
            ...
            for (int i = 0; i < array.size(); i++) {
                ...
                Long productId = productMap.get(item);
                ...
                if (priceBookEntryId != null && priceBookEntryId != 0) {
                    ...
                    if (priceBookId.longValue() == 8888888L) {
                        continue;
                    }
                    //todo 创建
                    JSONObject priceBookEntryObject = new JSONObject();
...
//            jsonArray.add("LIVE_HXSW");
        ...
        Map<Long, Long> SXDateMap = new HashMap<>();
        String priceBookSql = "select id,name,customItem1__c ,customItem3__c ,customItem6__c , ownerId, dimDepart,enableFlg,standardFlg from priceBook";
        JSONObject byXoqlSimple = queryServer.getByXoqlSimple(priceBookSql);
        JSONArray allByXoqlSample = queryServer.getAllByXoqlSample(getToken(), byXoqlSimple, priceBookSql);
        for (int i = 0; i < allByXoqlSample.size(); i++) {
            JSONObject jsonObject = allByXoqlSample.getJSONObject(i);
...
//                String productName = jsonObject.getString("productName");
        ...
        String priceBookEntrySql = "select id,productId,priceBookId,bookPrice,name,customItem1__c,customItem3__c,customItem10__c,customItem11__c from priceBookEntry";
        JSONObject byXoqlSimple2 = queryServer.getByXoqlSimple(priceBookEntrySql);
        JSONArray allByXoqlSample2 = queryServer.getAllByXoqlSample(getToken(), byXoqlSimple2, priceBookEntrySql);
        for (int i = 0; i < allByXoqlSample2.size(); i++) {
            JSONObject jsonObject = allByXoqlSample2.getJSONObject(i);
        ...
        String ERPAccountSql = "select id,dimDepart,customItem4__c,customItem2__c,name,customItem10__c ,customItem11__c from customEntity63__c";
        JSONObject byXoqlSimple3 = queryServer.getByXoqlSimple(ERPAccountSql);
        JSONArray allByXoqlSample3 = queryServer.getAllByXoqlSample(getToken(), byXoqlSimple3, ERPAccountSql);
        for (int i = 0; i < allByXoqlSample3.size(); i++) {
            JSONObject jsonObject = allByXoqlSample3.getJSONObject(i);
...
//                String productName = item + "-" + uf_standNum;
                ...
                Long accountId = object.getLong("customItem4__c");
                Long ERPId = object.getLong("id");
                String dimDepart = depMap.get(custNum);
                String ownerId = ownerMap.get(custNum);
                String accountName = object.getString("customItem10__c");
                Long productId = productMap.get(item);
                Long priceBookId = priceBookMap.get(config + "," + accountId);
                Long priceBookEntryId = priceBookEntryMap.get(productName + "," + productId + "," + priceBookId);
...
//                    priceBookEntryObject.put("customItem10__c",uf_standNum);
                    priceBookEntryMap.put(productName + "," + productId + "," + priceBookId, 88888L);
                    insertpriceBookEntryArray.add(priceBookEntryObject);
                }
...
//        System.err.println(pa.value);
    ...
    public String syncFreeSample(@RequestBody JSONObject param) {
        ...
        try {
            ...
            String string = JSONObject.toJSONString(workflowRequestInfo, SerializerFeature.WriteMapNullValue);
